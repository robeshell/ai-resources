...
import {
  ...
  Skeleton, Stack, Table, Tabs, Text, TextInput,
} from "@mantine/core";
...
function ContentIdentity({ item }: { item: LibraryItem }) {
  ...
  return <Link href={curatorEditorHref(item.blockType, item.slug)} className="curator-content-link" title={`${item.slug}${source ? ` · ${source}` : ""}`}>
    <Stack gap={3}>
      <Text fw={500} size="sm" c="dark.8" className="curator-library-title">{item.title}</Text>
...
export function ResourceLibrary() {
  ...
  return <Stack gap="sm" className="curator-library">
    <CuratorPageHeader title="资源库" description={loading ? "正在读取资源…" : `${result.counts.all} 条内容 · ${result.counts.active} 条已发布`} actions={<Button component={Link} href={`/curator/ingest/?block=${createBlock}`}>收录{BLOCK_LABELS[createBlock]}</Button>} />

    <Tabs value={block} onChange={(value) => updateParams({ block: value || "all", page: 1 })} variant="default" keepMounted={false}>
    ...
    <Paper component="form" withBorder p="md" onSubmit={submitSearch} className="curator-library-filters">
      <div className="curator-library-filter-grid">
        <TextInput value={search} onChange={(event) => setSearch(event.currentTarget.value)} placeholder="搜索名称、链接或摘要" aria-label="搜索资源" />
        ...
        <Checkbox label="仅看待补齐" checked={issues} onChange={(event) => updateParams({ issues: event.currentTarget.checked, page: 1 })} />
    ...
    <Paper withBorder p={0} className="curator-library-table-shell">
      {selected.length ? <Group justify="space-between" px="sm" py="xs" mih={40} className="curator-library-resultbar">
        <Group gap="sm"><Text size="sm" fw={500} role="status" aria-live="polite">{selected.length ? `已选择 ${selected.length} 条` : "内容列表"}</Text><Text size="sm" c="dimmed" className="curator-number">{range}</Text></Group>
        {selected.length ? <Group gap="xs"><Button loading={busy} variant="default" onClick={() => void batch("active")}>发布所选</Button><Button color="red" variant="subtle" disabled={busy} onClick={() => void batch("archived")}>归档所选</Button><Button variant="subtle" color="gray" disabled={busy} onClick={() => setSelected([])}>取消选择</Button></Group> : null}
      </Group> : null}
      {loading ? <Stack p="md" gap="xs">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} h={54} radius="sm" />)}</Stack> : null}
      {!loading && !result.items.length ? <Stack align="center" justify="center" mih={260} p="xl" gap="sm" className="curator-library-empty"><Text fw={500}>{hasFilters ? "没有匹配的内容" : "资源库还是空的"}</Text><Text size="sm" c="dimmed">{hasFilters ? "换个关键词，或者清除当前筛选。" : "从一个链接开始，Agent 会先整理成可检查的草稿。"}</Text><Group gap="xs">{hasFilters ? <Button variant="default" onClick={clearFilters}>清除筛选</Button> : null}<Button component={Link} href={`/curator/ingest/?block=${createBlock}`}>收录{BLOCK_LABELS[createBlock]}</Button></Group></Stack> : null}
      {!loading && result.items.length ? <>
        <Table.ScrollContainer minWidth={760} visibleFrom="sm"><Table verticalSpacing="xs" horizontalSpacing="sm" highlightOnHover>
          ...
          <Table.Tbody>{result.items.map((item) => <Table.Tr key={item.id} data-selected={selected.includes(item.id) || undefined}>
            ...
            <Table.Td><span className="curator-resource-status" data-status={item.status}>{STATUS_LABEL[item.status]}</span></Table.Td><Table.Td>{item.issueCount ? <Badge color="orange" circle>{item.issueCount}</Badge> : <Text c="dimmed">—</Text>}</Table.Td>
        ...
        <Stack hiddenFrom="sm" gap={0}>{result.items.map((item) => <Box key={item.id} p="md" className="curator-mobile-resource-row"><Group align="flex-start" wrap="nowrap">
          ...
          <Stack gap="xs" flex={1}><ContentIdentity item={item} /><Group gap="xs"><Badge color="gray" variant="light">{BLOCK_LABELS[item.blockType as Exclude<LibraryBlock, "all">]}</Badge><span className="curator-resource-status" data-status={item.status}>{STATUS_LABEL[item.status]}</span>{item.issueCount ? <Badge color="orange" variant="light">{item.issueCount} 个问题</Badge> : null}</Group><Group justify="space-between"><Text size="xs" c="dimmed" className="curator-number">更新于 {new Date(item.updatedAt).toLocaleDateString("zh-CN")}</Text><Button component={Link} href={curatorEditorHref(item.blockType, item.slug)} variant="subtle" size="compact-sm">编辑</Button></Group></Stack>
        </Group></Box>)}</Stack>
      </> : null}
      <Flex justify="space-between" align="center" gap="md" wrap="wrap" p="md" className="curator-pagination-mantine">
        ...
        <Text size="xs" c="dimmed" role="status">{range}</Text>
