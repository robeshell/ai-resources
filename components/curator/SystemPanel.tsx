  ...
  return <section className="curator-settings-page">
    ...
    <div className="curator-settings-layout">
      ...
      <div className="curator-settings-content">
        ...
        <Paper component="section" id="local-data" withBorder className="curator-settings-section"><header className="curator-settings-section-heading"><div><Text className="curator-settings-index">03</Text><Title order={2}>本地运行记录</Title><Text size="sm" c="dimmed">失败任务与未保存过程。</Text></div><Badge color="gray" variant="light">{records ? `${records.count} 份` : "—"}</Badge></header><div className="curator-settings-metrics"><div><Text size="xs" c="dimmed">数量</Text><Text fw={500} className="curator-number">{records?.count ?? "—"}</Text></div><div><Text size="xs" c="dimmed">最早</Text><Text fw={500} className="curator-number">{records?.oldest ? new Date(records.oldest).toLocaleDateString("zh-CN") : "—"}</Text></div><div><Text size="xs" c="dimmed">占用</Text><Text fw={500} className="curator-number">{records?.bytes ? `${(records.bytes / 1024).toFixed(1)} KB` : "—"}</Text></div></div><div className="curator-settings-danger"><div><Text fw={500} size="sm">清除运行记录</Text><Text size="xs" c="dimmed">不影响已保存资源。</Text></div><Button color="red" variant="light" onClick={() => setConfirmOpen(true)} disabled={busy || !records?.count}>清除记录</Button></div></Paper>
      </div>
    </div>
