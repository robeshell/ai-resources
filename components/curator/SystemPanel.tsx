...
export function SystemPanel() {
  ...
  return <section className="curator-settings-page">
    <CuratorPageHeader title="项目设置" description="当前项目的 AI、构建和本地数据。" />
    ...
    <div className="curator-settings-layout">
      ...
      <div className="curator-settings-content">
        <Paper component="form" id="ai-connection" withBorder className="curator-settings-section" onSubmit={savePiConfig}>
          <header className="curator-settings-section-heading"><div><Text className="curator-settings-index">01</Text><Title order={2}>Pi Agent 连接</Title><Text size="sm" c="dimmed">项目独立的网关与模型。</Text></div><Group gap="xs" wrap="wrap" justify="flex-end"><Text size="xs" c="dimmed" className="curator-settings-value">{agent?.defaultModelLabel || "未选择模型"}{gatewayModels.length ? ` · ${gatewayModels.length} 个模型` : ""}</Text><Badge color={piConfig?.configured ? "curator" : "gray"} variant="light">{piConfig?.configured ? "已配置" : "未配置"}</Badge></Group></header>
          <Stack gap="md" className="curator-settings-form">
        ...
        <Paper component="section" id="site-build" withBorder className="curator-settings-section"><header className="curator-settings-section-heading"><div><Text className="curator-settings-index">02</Text><Title order={2}>站点构建</Title><Text size="sm" c="dimmed">发布前检查公开页面。</Text></div><Badge color={build.status === "error" ? "red" : build.status === "ok" || build.status === "running" ? "curator" : "gray"} variant="light">{build.status === "running" ? "校验中" : build.status === "ok" ? "已通过" : build.status === "error" ? "失败" : "空闲"}</Badge></header><div className="curator-settings-build-body"><div>{build.error ? <Alert color="red" title="校验失败">{build.error}</Alert> : null}<Group><Button loading={busy || build.status === "running"} onClick={runBuildCheck}>运行构建校验</Button><Button component="a" href={build.publicUrl || "http://localhost:3000/zh/"} target="_blank" variant="default">打开公开站</Button></Group></div><Accordion variant="contained"><Accordion.Item value="log"><Accordion.Control>构建日志</Accordion.Control><Accordion.Panel><pre className="curator-code-block">{build.log?.trim() || "暂无构建输出"}</pre></Accordion.Panel></Accordion.Item></Accordion></div></Paper>

        <Paper component="section" id="local-data" withBorder className="curator-settings-section"><header className="curator-settings-section-heading"><div><Text className="curator-settings-index">03</Text><Title order={2}>本地运行记录</Title><Text size="sm" c="dimmed">失败任务与未保存过程。</Text></div><Badge color="gray" variant="light">{records ? `${records.count} 份` : "—"}</Badge></header><div className="curator-settings-metrics"><div><Text size="xs" c="dimmed">数量</Text><Text fw={600} className="curator-number">{records?.count ?? "—"}</Text></div><div><Text size="xs" c="dimmed">最早</Text><Text fw={600} className="curator-number">{records?.oldest ? new Date(records.oldest).toLocaleDateString("zh-CN") : "—"}</Text></div><div><Text size="xs" c="dimmed">占用</Text><Text fw={600} className="curator-number">{records?.bytes ? `${(records.bytes / 1024).toFixed(1)} KB` : "—"}</Text></div></div><div className="curator-settings-danger"><div><Text fw={600} size="sm">清除运行记录</Text><Text size="xs" c="dimmed">不影响已保存资源。</Text></div><Button color="red" variant="light" onClick={() => setConfirmOpen(true)} disabled={busy || !records?.count}>清除记录</Button></div></Paper>
      </div>
    </div>
