...
export function SystemPanel() {
  ...
  return <Stack gap="xl">
    ...
    {message ? <Alert color="teal" title="系统状态" role="status">{message}</Alert> : null}
    <SimpleGrid cols={{ base: 1, lg: 3 }} spacing="md">
      <Paper withBorder p="xl"><Group justify="space-between" mb="lg"><Box><Text className="curator-eyebrow-mantine">本机</Text><Title order={2} mt={4}>Agent</Title></Box><Badge color={agents.some((agent) => agent.available) ? "teal" : "red"} variant="light">{agents.filter((agent) => agent.available).length} 可用</Badge></Group>
        {!agents.length ? <Stack gap="xs"><Skeleton h={48} /><Skeleton h={48} /></Stack> : <Stack gap={0}>{agents.map((agent) => <Group key={agent.id} justify="space-between" py="sm" className="curator-system-row"><Box><Text fw={600} size="sm">{agent.label}</Text><Text size="xs" c="dimmed" mt={2}>{agent.available ? agent.defaultModel || "默认模型" : "不可用"}</Text></Box><Badge color={agent.available ? "teal" : "red"} variant="dot">{agent.available ? "正常" : "离线"}</Badge></Group>)}</Stack>}
      </Paper>

      <Paper withBorder p="xl"><Group justify="space-between" mb="lg"><Box><Text className="curator-eyebrow-mantine">公开站</Text><Title order={2} mt={4}>构建校验</Title></Box><Badge color={build.status === "error" ? "red" : build.status === "ok" ? "teal" : build.status === "running" ? "curator" : "gray"} variant="light">{build.status === "running" ? "校验中" : build.status === "ok" ? "已通过" : build.status === "error" ? "失败" : "空闲"}</Badge></Group>
