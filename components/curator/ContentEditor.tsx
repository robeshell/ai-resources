...
export function ContentEditor({ block, slug }: { block: EditableBlock; slug: string }) {
  ...
  async function save() {
    ...
    try { const normalized = { ...draft, id: draft.id || draft.slug, slug: draft.slug.trim(), title: draft.title.trim() }; const result = await curatorRequest<{ item: CuratorContentItem }>("/content", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ item: normalized, revisionId: original?.revision?.id }) }); setOriginal(result.item); setDraft(structuredClone(result.item)); if (isNew || slug !== result.item.slug) router.replace(`/curator/resources/${block}/${encodeURIComponent(result.item.slug)}`); }
  ...
  return <div className={conversationContentId ? "curator-editor-split" : undefined}>
    ...
    <Stack gap="lg" className="curator-editor-main" style={{ minWidth: 0 }}>
