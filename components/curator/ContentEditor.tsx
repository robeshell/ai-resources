...
import { BLOCK_LABELS, PHASE_LABEL, curatorEventUrl, curatorRequest, type CuratorContentItem, type CuratorRun, type CuratorRunEvent } from "@/lib/curator-client";
  ...
  return <Drawer opened={open} onClose={() => onOpenChange(false)} position="right" size="xl" title={<Box><Text fw={600}>Agent 重新处理</Text><Text size="sm" c="dimmed" mt={2}>只生成候选，接受后仍需在编辑器中保存。</Text></Box>} overlayProps={{ backgroundOpacity: 0.35, blur: 2 }}>
    <Stack gap="lg">
      ...
      {run ? <Paper withBorder p="md"><Stack gap="md">
        ...
        <Timeline active={events.length} bulletSize={18} lineWidth={2}>{events.map((event) => <Timeline.Item key={event.sequence} color={event.level === "error" ? "red" : event.level === "warning" ? "yellow" : "teal"} title={PHASE_LABEL[event.phase] ?? event.phase}><Text size="sm" c="dimmed">{event.message}</Text></Timeline.Item>)}</Timeline>
        {!events.length ? <Skeleton h={64} /> : null}
        {run.agent?.mode === "rules" ? <Alert color="yellow" title="规则草稿">Agent 未返回结构化结果，文案需要人工重写。</Alert> : null}
        ...
        {run.status === "failed" ? <Accordion variant="contained"><Accordion.Item value="error"><Accordion.Control>技术信息</Accordion.Control><Accordion.Panel><pre className="curator-code-block">{run.error || events.filter((event) => event.type === "tool.output").map((event) => event.message).join("\n")}</pre></Accordion.Panel></Accordion.Item></Accordion> : null}
      </Stack></Paper> : null}
