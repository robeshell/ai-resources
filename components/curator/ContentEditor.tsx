...
export function ContentEditor({ block, slug }: { block: EditableBlock; slug: string }) {
  ...
  const [propertiesOpen, setPropertiesOpen] = useState(isNew);
  ...
  async function save() {
    ...
    if (Object.keys(validation).length) { setPropertiesOpen(true); setError("请先处理表单中标出的内容"); focusFirstError(); return; }
  ...
  return <div className={conversationContentId ? "curator-editor-split" : undefined}>
    <h1 className="curator-visually-hidden">编辑 {draft.title || BLOCK_LABELS[block]}</h1>
    ...
    <Stack gap={0}>
      <details className="curator-editor-properties" open={propertiesOpen} onToggle={(event) => setPropertiesOpen(event.currentTarget.open)}>
        <summary><span>属性与分类</span><span className="curator-editor-property-summary">{BLOCK_LABELS[block]} · {{ draft: "草稿", active: "已发布", archived: "已归档" }[draft.status]}</span></summary>
      ...
      <EditorSection title="标签" description="补充描述这条内容，可多选。"><TagPicker value={draft.tags} onChange={(tags) => updateItem("tags", tags)} /></EditorSection>
        {tool ? <EditorSection title="工具入口" description="Logo 与主要访问入口。"><SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md"><TextInput label="Logo 路径" value={tool.logo || ""} onChange={(event) => updatePayload({ ...tool, logo: event.currentTarget.value })} placeholder="/logos/…" /><TextInput label="官网链接" value={tool.url} error={fieldErrors.toolUrl} onChange={(event) => updatePayload({ ...tool, url: event.currentTarget.value })} /></SimpleGrid></EditorSection> : null}
        {site ? <EditorSection title="站点入口" description="公开访问地址与可选图标。"><SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md"><TextInput label="Logo 路径" value={site.logo || ""} onChange={(event) => updatePayload({ ...site, logo: event.currentTarget.value })} placeholder="/logos/…" /><TextInput label="站点地址" value={site.url} error={fieldErrors.siteUrl} onChange={(event) => updatePayload({ ...site, url: event.currentTarget.value })} /></SimpleGrid></EditorSection> : null}
      </details>
      {tool ? <>
