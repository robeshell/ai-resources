...
import { Button, Group, Paper, Select, SimpleGrid, Stack, TextInput, Textarea } from "@mantine/core";
...
export function StructuredLinks({ value, onChange }: { value: ContentLink[]; onChange: (next: ContentLink[]) => void }) {
  return <Stack gap="sm">
    {value.map((link, index) => <Paper withBorder p="sm" key={`${index}-${link.url}`}>
